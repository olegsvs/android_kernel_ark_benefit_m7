...
/*----------------------------------------------------------------------------*/
static int mmc3416x_init_flag = 0;
/*----------------------------------------------------------------------------*/
...
static int MMCECS_CheckDevice(void)
{
	char buffer[2];
	int ret;
	printk(" mmc3416x check device id\n");
	/* Set measure mode */
	buffer[0] = MMC3416X_REG_PRODUCTID_1;

	/* Read data */
	ret = I2C_RxData(buffer, 1);
	printk(" mmc3416x check device id = 0x%x\n",buffer[0]);
	printk(" mmc3416x ret = %d\n",ret);
	if(ret < 0)
	{
		return ret;
	}
...
//	    if ((err = mmc3416x_SetPowerMode(obj->client, ture)) < 0) {
//		    MAG_ERR("%s:MMC3416x_SetMode Error.\n", __FUNCTION__);
//		    return err;
//		}
	}
	else
	{
		...
		{
			atomic_set(&open_flag, 0);
//		    if ((err = mmc3416x_SetPowerMode(obj->client, false)) < 0) {
//		    MAG_ERR("%s:MMC3416x_SetMode Error.\n", __FUNCTION__);
//		    return err;
//			}
		}
...
static int mmc3416x_m_set_delay(u64 ns)
{
...
static int mmc3416x_m_open_report_data(int open)
{
	return 0;
}

static int mmc3416x_m_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int mmc3416x_o_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= 10)
...
static int mmc3416x_o_open_report_data(int open)
{
	return 0;
}

static int mmc3416x_o_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
//modified for mmc3416x----changhua start
	/* Check connection */
	err = MMCECS_CheckDevice();
	if(err < 0)
	{
		MAG_ERR("mmc3416x probe: check device connect error\n");
...
*/
	...
	ctl.m_enable = mmc3416x_m_enable;
	ctl.m_set_delay  = mmc3416x_m_set_delay;
	ctl.m_open_report_data = mmc3416x_m_open_report_data;
	ctl.o_enable = mmc3416x_o_enable;
	ctl.o_set_delay  = mmc3416x_o_set_delay;
	ctl.o_open_report_data = mmc3416x_o_open_report_data;
...
*/
	...
	mag_data.get_data_o = mmc3416x_o_get_data;
	mag_data.get_data_m = mmc3416x_m_get_data;
...
static int mmc3416x_local_init(void)
{
	...
	{
		printk(KERN_ERR "mmc3416x add driver error\n");
		return -1;
	} 
